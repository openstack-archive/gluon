...
def process_base_port_port(message, uuid):
    ...
    if action == 'set' or action == 'update':
        ...
        # TODO(enikher) get this from nova somehow
        ...
        odlc.delete_ietf_interface(name=uuid)
...
def process_vpn_instance(message, uuid):
    ...
    if action == 'set' or action == 'update':
        message_value = json.loads(message.value)
        # TODO(enikher) check is it belongs to us
        ipv4_vpnTargets = []
        if 'ipv4_family' in message_value:
            # TODO(enikher) make lists possible as well
            vpn_af_config = json.loads(
                etcd_client.get(proton_etcd_dir +
                                '/VpnAfConfig/' +
                                message_value['ipv4_family']).value)
            ipv4_vpnTargets = {'vrfRTType': vpn_af_config['vrf_rt_type'],
                               'vrfRTValue': vpn_af_config['vrf_rt_value']}
        odlc.update_l3_vpn_instance(
            name=message_value['id'],
...
def process_message(message):
    try:
        ...
        LOG.error(
            'Error when processing message: %s. ex: %s' % (message, str(ex)))
...
def getargs():
    ...
    parser.add_argument('-H', '--host-name',
                        required=False,
                        help='etcd hostname or ip, default to localhost',
                        dest='etcd_host', type=str)
    parser.add_argument('-p', '--port',
                        required=False,
                        help='etcd port number, default to 2379',
                        dest='etcd_port',
                        type=str)
...
def full_sync(etcd_client, messages_queue):
    ...
    for child in root.get_subtree(leaves_only=True):
        ...
        for message in etcd_client.get(child.key).get_subtree(
                leaves_only=True):
...
def main():
    ...
    while True:
        try:
            ...
            if wait_index:
                message = etcd_client.read(proton_etcd_dir,
                                           recursive=True,
                                           wait=True,
