...
class ProtonManager(ApiManager):
    def __init__(self):
        ...
        self.wait_index = 0
        self.etcd_client = etcd.Client(host=ManagerData.etcd_host, port=ManagerData.etcd_port,
                                       read_timeout=2)
        super(ProtonManager, self).__init__()

    def setup_bind_key(self, key):
        etcd_key = "{0:s}/{1:s}/{2:s}/{3:s}".format("controller", self.service, "ProtonBasePort", key)
        #
    ...
    def wait_for_bind(self, key):
        ...
        while retry > 0:
            try:
                ...
                message = self.etcd_client.read(etcd_key, wait=True, waitIndex=self.wait_index)
    ...
    def update_baseports(self, api_class, obj_class, key, new_values):
        bind_attrs = ("host_id" in new_values and "device_id" in new_values)
        bind_request = (bind_attrs and  new_values["host_id"] != "" and new_values["device_id"] != "")
        if bind_request:
        ...
        if bind_request:
            ...
            if len(vif_dict) == 0:
                ...
                if ("vif_details" in vif_dict):
                    new_values["vif_details"] = json.dumps(vif_dict["vif_details"])
                if len(new_values) > 0:
                    ret_obj = api_class.build(obj_class.update(key, new_values))
        elif bind_attrs: #unbind request
            new_values["vif_type"] = ""
