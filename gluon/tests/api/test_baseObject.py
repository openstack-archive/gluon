...
class APIBaseListTestCase(base.APITestCase):
    ...
    @patch('gluon.api.baseObject.dbapi.get_instance')
    def test_class_builder(self, mock_get_instance):
        _db_model = type("FooDbCls", (object, ), {"foo": str})
        api_object_class = baseObject.APIBaseObject.class_builder(
            "FoodAPICls", _db_model, {"foo": str})
        class_name = 'FooListAPICls'
        list_name = "listOfFoo"
...
class RootObjectControllerTestCase(base.APITestCase):
    ...
    test class_builder
    """
    @patch('gluon.api.baseObject.get_api_manager')
    ...
    def test_class_builder(self,
        ...
        """
        mock_wsexpose.assert_called_once_with(list_object_class,
                                              template='json')
        print("Hi")
        controller_cls.get_all()
        list_object_class.build.assert_called_once()
        """
